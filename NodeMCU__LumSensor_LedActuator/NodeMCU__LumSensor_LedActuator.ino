...
// WIFI params
char* WIFI_SSID = "GalaxyS20";    // Configure here the SSID of your WiFi Network
char* WIFI_PSWD = "zxla4008"; // Configure here the PassWord of your WiFi Network
int WIFI_DELAY  = 100; //ms

// oneM2M : CSE params
String CSE_IP      = "192.168.250.192" ; //"192.168.1.35"; //Configure here the IP Address of your oneM2M CSE
int   CSE_HTTP_PORT = 8282;
...
// Method for creating an ApplicationEnode onem2m-monitor.jsntity(AE) resource on the remote CSE (this is done by sending a POST request)
...
String createSUB(String ae, String CNT) {
  String subRepresentation =
    "{\"m2m:sub\": {"
    "\"rn\":\"SUB_" + ae + "\","
    "\"nu\":[\"http://127.0.0.1:1400\"], "
    "\"nct\":1"
    "}}";
  return doPOST("/" + CSE_NAME + "/" + ae + "/" + CNT, TY_SUB, subRepresentation);
}
    ...
    if (result == HTTP_CREATED) {
      ...
      if (isActuator) {
        ...
        if (result == HTTP_CREATED) {
          ...
          //result = createSUB(module, CMND_CNT_NAME);
          //if (result == HTTP_CREATED) {
#ifdef DEBUG
            //Serial.println("SUB " + module + "/" + CMND_CNT_NAME + "/SUB_" + module + " created !");
#endif
...
void task_luminosity() {
  ...
  String ciContent = "{value : "+String(sensorValue)+",unit : lux, sensor : luminosity, date : "+France.dateTime()+"}";
